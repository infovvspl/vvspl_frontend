...
const ventures = [
    ...
    { 
        ...
        img: "https://images.unsplash.com/photo-1598566986668-89a527817e9b?q=80&w=1000&auto=format&fit=crop", 
...
const Ventures = ({ onNextClick }) => {
    ...
    return (
        <section 
            ref={sectionRef} 
            id="ventures" 
            className="relative w-full min-h-screen bg-[#020205] text-white py-20 sm:py-24 px-6 md:px-12 flex flex-col justify-center overflow-hidden"
        >
            {/* Background Texture */}
            <div className="absolute inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-10 pointer-events-none" />

            {/* Header */}
            <div className="relative z-30 mb-12 sm:mb-16 text-center md:text-left">
            ...
            >
                {ventures.map((venture, index) => (
                    <div
                        key={index}
                        className="group relative h-[350px] sm:h-[450px] md:h-[500px] rounded-xl overflow-hidden cursor-pointer shadow-2xl bg-neutral-900"
                    >
                        {/* Background Image */}
                        <img 
                            src={venture.img} 
                        ...
                        <div className="absolute inset-0 p-6 sm:p-8 flex flex-col justify-end">
                            ...
                            <p className="text-neutral-300 text-xs sm:text-sm mb-6 opacity-0 group-hover:opacity-100 transform translate-y-4 group-hover:translate-y-0 transition-all duration-300 delay-75 line-clamp-3">
                                {venture.desc}
                            </p>
            ...
            <div className="relative z-30 mt-12 sm:mt-16 md:mt-20 w-full flex justify-center">
                <button
                    onClick={handleNext}
                    className="group relative px-8 py-3 sm:px-10 sm:py-4 bg-transparent border border-neutral-800 text-neutral-500 font-mono text-[10px] sm:text-xs tracking-[0.2em] hover:border-blue-500 hover:text-blue-400 transition-all duration-300 rounded-full overflow-hidden"
                >
                    <span className="relative z-10 flex items-center gap-2">
                       NEXT STATION - TO BLOGS
                        <span className="group-hover:translate-x-1 transition-transform">&rarr;</span>
                    </span>
                    <div className="absolute inset-0 bg-blue-500/5 transform -translate-x-full group-hover:translate-x-0 transition-transform duration-300" />
                </button>
            </div>
        </section>
    );
